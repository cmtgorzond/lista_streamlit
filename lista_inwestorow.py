...
class RocketReachAPI:
    ...
    def _handle_rate_limit(self, response):
        """Automatyczna obsługa limitu z czasem zwrotnym od serwera"""
        if response.status_code == 429:
            # Spróbuj odczytać z JSON pola 'wait'
            retry_after = None
            try:
                error_data = response.json()
                retry_after = float(error_data.get('wait'))
            except Exception:
                pass

            # Jeśli nie ma w body, spróbuj nagłówka Retry-After
    ...
    def search_people_with_emails(self, company_url: str, titles: List[str], exclude_titles: List[str]) -> List[Dict]:
        """Optymalizowane wyszukiwanie z batchowaniem i dwuetapowym procesem - 3 kontakty"""
        try:
            valid_contacts = []
            ...
            # Pobierz szczegóły dla wyników z stanowisk (batch lookup)
            if title_results:
                ids = [person['id'] for person in title_results[:15]]
                details_batch = self._batch_lookup_people(ids)
            ...
            if len(valid_contacts) < 3:
                ...
                # Pobierz szczegóły dla wyników z skills (pomijając już znalezione)
                existing_ids = {contact.get('id') for contact in valid_contacts if 'id' in contact}
                
                if skill_results:
                    new_ids = [person['id'] for person in skill_results[:15] if person['id'] not in existing_ids]
                    if new_ids:
                        details_batch = self._batch_lookup_people(new_ids)
    ...
    def _batch_lookup_people(self, person_ids: List[int]) -> List[Dict]:
        """Batch lookup dla wielu osób naraz (optymalizacja API)"""
        try:
            if not person_ids:
                return []
            
            # Jeśli mamy co najmniej 10 ID, użyj bulk lookup
            if len(person_ids) >= 10:
                self._rate_limit_check()
                
                payload = {"ids": person_ids[:100]}  # Maksymalnie 100 ID na raz
                ...
                while retry_count < max_retries:
                    response = requests.post(
                        f"{self.base_url}/api/v2/person/bulk_lookup",
                        headers=self.headers,
                        json=payload
                    )
                    ...
                    if self._handle_rate_limit(response):
                        ...
                        for person_data in data.get('results', []):
                            processed = self._process_email_data(person_data, person_data.get('id'))
                            if processed:
                        ...
                        st.error(f"Błąd bulk lookup API: {response.status_code} - {response.text}")
                        break
                
                return []
            
            else:
                # Fallback do pojedynczych lookup dla mniej niż 10 ID
                results = []
                for person_id in person_ids[:3]:  # Ogranicz do 3 dla oszczędności API
            ...
            st.error(f"Błąd batch lookup: {str(e)}")
            return []

    def lookup_person_details(self, person_id: int) -> Dict:
        """Pojedyncze pobieranie danych osoby"""
        try:
            self._rate_limit_check()
    ...
    if websites and api_key:
        if st.button("🚀 Rozpocznij wyszukiwanie", type="primary"):
            ...
            for i, website in enumerate(websites):
                ...
                # Optymalizowane wyszukiwanie z automatycznym sprawdzaniem emaili (3 kontakty)
